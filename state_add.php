            ...
            <div class="col-md-10 col-sm-10">
              ...
              <nav aria-label="breadcrumb" role="navigation">
                <ol class="breadcrumb">
                  ...
                  <li class="breadcrumb-item"><a href="state_view.php">State</a></li>
                  <li class="breadcrumb-item active" aria-current="page">Add State</li>
                </ol>
              </nav>
            </div>
						<div class="col-md-2 col-sm-2" align="right">
              <!-- <a  href="#" data-toggle="modal" data-target="#bill_head_add" type="button"><button  class="btn btn-primary">Add State</button></a> -->
              <a  href="state_view.php" ><button  class="btn btn-primary">View State</button></a>
            </div>
					</div>
				</div>
...
<div class="pd-20 bg-white border-radius-4 box-shadow mb-30">
                ...
                {                    
                    ...
                    $total_taluka = count($_POST['fld_state_name']);

                    for($i=0;$i<$total_taluka;$i++)
                    {
                      if(($_POST['fld_state_name'][$i]!=""))
                      {

                        $nm=mysqli_query($connect,"select * from tbl_state where fld_country_id='".$_POST['country']."' and fld_state_name = '".$_POST['fld_state_name'][$i]."' and fld_state_delete='0' ");
                        if(mysqli_num_rows($nm)>0)
                        {
                            echo "<script>";
                            echo "alert('State Is Already Exists');";
                        ...
                        {
                          $state1=$_POST['fld_state_name'][$i];
                          $state=ucwords(strtolower($state1));

                          $query = "INSERT INTO tbl_state(fld_country_id,fld_state_name,fld_state_delete) VALUES ('".$_POST['country']."','".$state."','0')" ;
                          ...
                          if(!empty($result)) 
                              ...
                              {
                                  echo "<script>";
                                   echo "alert('State Not Added Successfully');";
                                  echo "window.location.href='state_view.php';";
                                  echo "</script>";
                              }
                        }
            ...
            <div class="form-group row">              
              <div class="col-sm-12 col-md-12">
                <!-- <input class="form-control" type="text" name="bill_head" id="bill_head" placeholder="Enter State " required=""  oninput="this.value = this.value.replace(/[^A-Za-z\s]/g, '').replace(/(\..*)\./g, '$1');" > -->
                <table  class="table table-hover small-text" id="tb">
                  ...
                  <tr>
                    ...
                    <td><input type="text" name="fld_state_name[]" id="fld_state_name" placeholder="Enter State" class="form-control" required="" oninput="this.value = this.value.replace(/[^A-Za-z\s]/g, '').replace(/(\..*)\./g, '$1');" style="text-transform: capitalize;"></td>
                    </tr>                    
                  </table>
              </div>
            </div>
...
<!-- 							<label class="col-sm-12 col-md-3 col-form-label">Submit</label>
 -->							<div class="col-sm-12 col-md-9">
							<center><input class="btn btn-success" value="Submit" type="submit" name="submit">
								...
								<a href="state_view.php"><input class="btn btn-warning" value="Back" type="button"></a>
							</center>
							</div>
						</div>
					</form>
